...
int main(void) {
  @autoreleasepool {
    NSString *evidenceDirectory = @"evidence/issue-62";
    NSString *source = [evidenceDirectory stringByAppendingPathComponent:@"layer2-device.pdf"];
    ...
    NSArray<NSNumber *> *pageNumbers = @[@1, @2, @3, @4, @6, @7];
    CGFloat scale = 2.0;
    ...
    for (NSNumber *number in pageNumbers) {
      ...
      NSString *screenshot = [evidenceDirectory stringByAppendingPathComponent:
        [NSString stringWithFormat:@"layer2-device-pdf-page-%ld.png", (long)pageNumber]];
    ...
    NSDictionary *result = @{
      ...
      @"manualInspectionRequired": @[
        ...
        @"Sections A and B agree with the reviewed patient and device event",
        @"Section C marks the device available for evaluation",
        @"Section D is empty and the suspect device appears in Section E",
