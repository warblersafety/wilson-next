...
test("Draft 4 retains drafts and proposals across screens, revisits reporter details, and displays the real current or earlier PDF", async ({ page }, info) => {
  ...
  await expect(page.getByLabel("Useful clinical details")).toContainText("Treatment history for amoxicillin");
  await expect(page.getByLabel("Useful clinical details")).toContainText("Review the groups above before sending another update.");
  ...
  await expect(page.getByRole("heading", { name: "Add or correct information (optional)", exact: true })).toBeVisible();
  await goTo(page, "Reporter details");
  await expect(page.getByLabel("Reporter email", { exact: true })).toHaveValue("casey@example.test");
