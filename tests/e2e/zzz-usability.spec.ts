...
test("focused usability: truthful activity, draft demo reporter, review dependencies and regenerated PDFs", async ({ page }, info) => {
  ...
  const spinner = openingButton.locator('[aria-hidden="true"]');
  ...
  const acceptBox = await event.getByRole("button", { name: "Accept event details", exact: true }).boundingBox();
  const rejectBox = await event.getByRole("button", { name: "Discard proposed event details", exact: true }).boundingBox();
  ...
  await page.getByRole("button", { name: "Review remaining details", exact: true }).click();
  ...
  await test2.getByRole("link", { name: "Go to proposed changes" }).click();
  ...
  await expect(test2).not.toContainText("Review the proposed changes above to enable these controls.");
  await acceptOpeningGroups(page);
  await goTo(page, "Reporter details");
