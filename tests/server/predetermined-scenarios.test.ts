...
describe("predetermined E2E scenario ownership", () => {
  it("assembles the exact attributable 26-call queue from one authored source", () => {
    ...
    expect(assembled).toHaveLength(26);
    expect(predeterminedModelResponses).toEqual(assembled);
    expect(new Set(assembled.map(({ identityScope }) => identityScope)).size).toBe(26);
  });
});
