...
describe("predetermined E2E scenario ownership", () => {
  it("assembles the exact attributable 39-call queue from one authored source", () => {
    ...
    expect(assembled).toHaveLength(39);
    ...
    expect(assembled.slice(-3).map(({ identityScope }) => identityScope)).toEqual(["issue106", "issue106-update", "issue109-update"]);
    expect(new Set(assembled.map(({ identityScope }) => identityScope)).size).toBe(34);
  });
});
